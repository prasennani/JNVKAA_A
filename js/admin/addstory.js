...
function addStories() {
    ...
    $.ajax({
        url: '../WebService.asmx/addStories',
        type: "POST", // type of the data we send (POST/GET)
        contentType: "application/json",
        data: "{ 'storytitle': '" + $('#title').val() +  "', 'postedby': '" + $('#postedby').val() + "', 'description1': '" + $('#description1').val() + "', 'description2': '" + $('#description2').val() + "', 'description3': '" + $('#description3').val() +  "', 'photo': '" + base64String + "'}",
        datatype: "json",
        success: function (response) { // when successfully sent data and returned
            ...
            switch (parseInt(JSON.parse(response.d))) {
                case 1:
                    ...
                    alert("Story Inserted");


                    break;
                case 0:
                    $('#preloader').css('display', 'none');
                    alert("Unable to Insert Story Data. Try after sometime.");
                    break;
